...
class Requisition_model extends CI_Model
{
	...
	{
		$sql = $this->db->query("SELECT reqitemsid,requisition_items.itemno,requisition_items.unit,items.description,qty,update_status from requisition_items left join items on requisition_items.itemno = items.itemNo where requisition_no='$requisition_no'");
		return $sql->result_array();
	...
	public function updateinventory($reqid)
	{
		...
		foreach ($reqitems as $req_items):
			...
			//update inventory per item
				...
				$sql5 = $this->db->query("UPDATE requisition_items set update_status=1 where itemNo=$itemno AND reqid='$reqid'");
