...
class Purchaserequest_model extends CI_Model
{
	...
	public function get()
	{
		$users = $this->db->query("SELECT purchase_pr.prid,prdate,aprno,aprdate,purchase_pr.prno,department,section,pono FROM purchase_pr LEFT JOIN purchase_po ON purchase_pr.prid = purchase_po.prid");
	...
	{
		$sql = $this->db->query("SELECT * FROM purchase_pr LEFT JOIN suppliers ON purchase_pr.awardedsupplier = suppliers.supplierID WHERE prid='$id'");
		$prmain = $sql->result_array();
		return $prmain[0];
	}
	
	public function savepr($prdate,$aprno,$prno,$department,$section,$purpose,$modeofprocurement)
	{
		$this->load->library('session');
		$this->session;
		$uid = $this->session->userdata('uid');
		
		$sql = "INSERT INTO purchase_pr (prdate,aprno,prno,department,section,purpose,addedby,modeofprocurement) VALUES (".$this->db->escape($prdate).", ".$this->db->escape($aprno).", ".$this->db->escape($prno).", ".$this->db->escape($department).", ".$this->db->escape($section).",".$this->db->escape($purpose).", ".$this->db->escape($uid).", ".$this->db->escape($modeofprocurement).")";
		$this->db->query($sql);
	...
	public function saveprdetails($prid,$department,$aprno,$aprdate,$section,$prno,$prdate,$purpose,$modeofprocurement)
	{
		$this->load->library('session');
		$this->session;
		$uid = $this->session->userdata('uid');
		
		$sql = "update purchase_pr set department=".$this->db->escape($department).",aprno=".$this->db->escape($aprno).",aprdate=".$this->db->escape($aprdate).",section=".$this->db->escape($section).",prno=".$this->db->escape($prno).",prdate=".$this->db->escape($prdate).",purpose=".$this->db->escape($purpose).",modeofprocurement=".$this->db->escape($modeofprocurement)." where prid=".$this->db->escape($prid)."";
	...
	public function updatecanvassprice($aocid,$unitprice)
	{
				
		$sql = "update purchase_pr_aoc set unitprice=".$this->db->escape($unitprice)." where aocid=".$this->db->escape($aocid)."";
